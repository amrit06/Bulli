...
<?php
if ($_SESSION['status'] === "admin") {
echo '  <a href="socialadd.php" class="button">Create New Entry</a>';
}
?> 
  ...
  <table class="center" style="width:80%" id="myTable">
    ...
    $table = new Table($db);
    $result = $table->retrieveall('Social');
    $size = sizeof($result);
    //print_r($result);
    for ($i = 0; $i < sizeof($result); $i++) {
      ...
      echo
        '
              ...
              <td>
              '. (empty($result[$i]['invoice']) ? '' : '<a href="../invoice.php?invoice='.$result[$i]['Id'].'&& table=Social ">PDF</a>' ).'
              </td>';
              if ($_SESSION['status'] === "admin") {
                echo '
              <td><a href="socialmodify.php?id='.$result[$i]['Id'].'" class="button2">Edit Entry</a></td>
              </tr>
              ';}
    }
