...
<?php
if ($_SESSION['status'] === "admin") {
echo '  <a href="cashadd.php" class="button">Create New Entry</a>';
}
?>  
  <input type="text" id="myInput" onkeyup="searchAll()" placeholder="Search for Entry" title="Type in a name" style="margin-left: 947px">
    ...
    for ($i = 0; $i < sizeof($result); $i++) {
      ...
      echo
        '
              ...
              <td>
              '. (empty($result[$i]['invoice']) ? '' : '<a href="../invoice.php?invoice='.$result[$i]['Id'].'&& table=Cash ">PDF</a>' ).'
              </td>';
              if ($_SESSION['status'] === "admin") {
              echo '
              <td><a href="cashmodify.php?id='.$result[$i]['Id'].'" class="button2">Edit Entry</a></td>
              </tr>
              ';}
    }
