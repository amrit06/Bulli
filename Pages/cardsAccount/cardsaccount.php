...
<?php
if ($_SESSION['status'] === "admin") {
echo '  <a href="cardsadd.php" class="button">Create New Entry</a>';
}
?>
    ...
    for ($i = 0; $i < sizeof($result); $i++) {
      ...
      echo
        '
              ...
              <td>' . $result[$i]['User'] . '</td>
              <td>
              '. (empty($result[$i]['invoice']) ? '' : '<a href="../invoice.php?invoice='.$result[$i]['Id'].'&& table=Card ">PDF</a>' ).'
              </td>';
              if ($_SESSION['status'] === "admin") {
                echo '
              <td><a href="cardsmodify.php?id='.$result[$i]['Id'].'" class="button2">Edit Entry</a></td>
              </tr>
              ';}
    }
