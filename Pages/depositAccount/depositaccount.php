...
<?php
if ($_SESSION['status'] === "admin") {
echo '  <a href="depositadd.php" class="button">Create New Entry</a>';
}
?>  <input type="text" id="myInput" onkeyup="searchAll()" placeholder="Search for Entry" title="Type in a name" style="margin-left: 947px">
  <table class="center" style="width:80%" id="myTable">
    ...
    $table = new Table($db);
    $result = $table->retrieveall('Deposit');
    $size = sizeof($result);
    //print_r($result);
    for ($i = 0; $i < sizeof($result); $i++) {
      ...
      echo
        '
              ...
              if ($_SESSION['status'] === "admin") {
                echo '
              <td><a href="depositmodify.php?id='.$result[$i]['Id'].'" class="button2">Edit Entry</a></td>
              </tr>
              ';}
    }
